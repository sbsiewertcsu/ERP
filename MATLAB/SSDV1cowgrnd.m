...
% Once (YOLO v2), and SSD. This example trains an SSD cow detector using the 
...
%end
%% Load Dataset
% This example uses a small cow data set that contains 295 images. Each 
% image contains one or two labeled instances of a cow. A small data set is 
...
rng(0);
shuffledIndices = randperm(height(cowDataset));
idx = floor(0.6 * length(shuffledIndices) );
trainingData = cowDataset(shuffledIndices(1:idx),:);
testData = cowDataset(shuffledIndices(idx+1:end),:);
%% 
