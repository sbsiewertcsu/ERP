...
% Once (YOLO v2), and SSD. This example trains an SSD car detector using the 
...
%end
%% Load Dataset
% This example uses a small car data set that contains 295 images. Each 
% image contains one or two labeled instances of a car. A small data set is 
...
data = load('satVehicleDatasetGroundTruth.mat');
satvehDataset = data.gTruth;
%% 
...
rng(0);
shuffledIndices = randperm(height(satvehDataset));
idx = floor(0.6 * length(shuffledIndices) );
trainingData = satvehDataset(shuffledIndices(1:idx),:);
testData = satvehDataset(shuffledIndices(idx+1:end),:);
%% 
...
bldsTrain = boxLabelDatastore(trainingData(:,'car'));

imdsTest = imageDatastore(testData{:,'imageFilename'});
bldsTest = boxLabelDatastore(testData(:,'car'));
%% 
...
numClasses = width(satvehDataset)-1;
%% 
...
[bboxes,scores] = detect(detector,I, 'Threshold', 0.55);
%% 
% Display the results.
