...
% Once (YOLO v2), and SSD. This example trains an SSD ele detector using the 
...
%end
%% Load Dataset
% This example uses a small ele data set that contains 295 images. Each 
% image contains one or two labeled instances of a ele. A small data set is 
% useful for exploring the SSD training procedure, but in practice, more labeled 
% images are needed to train a robust detector.

%unzip eleDatasetImages.zip
data = load('eleDatasetGroundTruth.mat');
eleDataset = data.gTruth;
%% 
% The training data is stored in a table. The first column contains the path 
% to the image files. The remaining columns contain the ROI labels for eles. 
...
rng(0);
shuffledIndices = randperm(height(eleDataset));
idx = floor(0.6 * length(shuffledIndices) );
trainingData = eleDataset(shuffledIndices(1:idx),:);
testData = eleDataset(shuffledIndices(idx+1:end),:);
%% 
...
% Adjusted intialLearn rate to 0.001 or less from 0.1
% Adjusted MiniBatchSize from 16 to 8

% Training with initialLearn rate=1e-5, MiniBatchSize=8
...
[bboxes,scores] = detect(detector,I, 'Threshold', 0.7);
%% 
% Display the results.
